...
import com.example.multidemo.IKeepAliveAidlInterface
import com.example.multidemo.R


class LocalForegroundService : Service() {

    private val kTag = "LocalForegroundService"
    ...
    private fun bindRemoteService() {
        ...
        // 绑定进程
        bindService(bindIntent, RemoteServiceConnection(), BIND_AUTO_CREATE)
    }

    inner class RemoteServiceConnection : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            Log.d(kTag, "onServiceConnected: ")
        }

        override fun onServiceDisconnected(name: ComponentName?) {
            Log.d(kTag, "onServiceDisconnected: ")
            startForegroundService()
            bindRemoteService()
        }
    }
