...
class FaceDetectActivity : KotlinBaseActivity<ActivityFaceDetectBinding>() {
    ...
    private val faceDetectorOptions by lazy {
        FaceDetectorOptions.Builder().apply {
            setLandmarkMode(FaceDetectorOptions.LANDMARK_MODE_NONE) // 不检测面部关键点（如眼睛、鼻子等）
            setClassificationMode(FaceDetectorOptions.CLASSIFICATION_MODE_NONE) // 不启用分类（如是否微笑、戴眼镜等）
            setPerformanceMode(FaceDetectorOptions.PERFORMANCE_MODE_ACCURATE)// 启用高精度模式（适合需要准确 bounding box 的场景）
        }.build()
    }
